    ...
    <form name='adminUsers' action='".$_SERVER['PHP_SELF']."' method='post'>
        <table class='table' id='example1'> 
            <thead>
                <tr>
                    <th>Funcionário</th>
                    <th>Total de Consulta(sistema)</th>
                </tr>
            </thead>";
            $con = connection();
            $data_hoje = date("d-m-Y");
				
				$sql = "SELECT tbl_users.display_name,SUM(total) AS Total FROM tbl_facturacao INNER JOIN tbl_users ON (tbl_facturacao.funcionario=tbl_users.id) WHERE tipoServico='gerais'";	
