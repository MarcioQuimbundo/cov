...
echo "
	<div id='page-content-wrapper'>
		...
		<h3>Isenção dos Serviços de Exames</h3>
	</div>
	...
	if (isset($_POST['dar_entradaa'])) {
		...
		$query = "INSERT INTO tbl_isencao_servicos_exames VALUES(NULL,'$idPaciente','$tipo_servicos','$preco','$quantidade','$total','$descricao','$user_id', '$data_isentada')";	
		$result = mysqli_query($con,$query);
		$q = "SELECT * FROM tbl_isencao_servicos_exames";	
			 ...
			 echo "	<div class='row'>
			                <div class='col-md-6'  style='margin-top:15%;margin-left:25%;'>
			                    <div class='infobox success-bg mrg0A text-center'>
			                        <h2>Paciente Isentado com sucesso</h2><br>
			                        <a href='imprimir/imprimir_isencao_servicos_exames.php?Id=".$idPaciente."&uid=".$count."' target='_blank' class='btn primary-bg large' name='isencao'>
			            ...
			            die();
		}else echo "<script>swal('Ops! Ocorreu um erro ao Isentar','','error');</script>";
 	...
 	if(isset($_GET['Id']))
        {
	        ...
	        if ($acao == 'insencao') {
				...
				echo "
			<a href='insesao_servicos_exames.php' style='padding-right: 10px; float:center; font-size:20px; background-color: #149900; color: #fff;' class='btn small'>
			...
			<form name='#' class='form-bordered' action='".$_SERVER['PHP_SELF']."' method='post'>
			              ...
			              ";?>
		              			...
		              			<div class='form-input col-md-6'>
									<select required='required' id='tipo_servicos' onchange="mudaPreco();" name='tipo_servicos' class='form-control' style="font-size: 14px;">
										<option value='' selected='selected'>-- Escolha um Serviço de Exames --</option>  
										<?php  
											$query="SELECT id, nome FROM tbl_servicos_exames";
											$result=mysqli_query($con,$query);
											while($row=mysqli_fetch_array($result))
											{
										?>
										<option value=<?="exames:".$row['id']; ?> ><?=$row['nome']; ?></option>        
										<?php } ?>                           
									</select>
								</div>
							</div>
			            ...
			            <script type="text/javascript">
							...
							function calcularTotal() {
							    ...
							    var total = parseFloat(quantidade.value) * add; 
								swal(total);
...
<table class='table' id='example1'>
	...
	$con=connection();
	$query="SELECT Id, nome, (TIMESTAMPDIFF(YEAR, data_nasc, CURDATE())) AS idade, genero, telefone FROM tbl_paciente inner join tbl_agenda ON tbl_paciente.Id = tbl_agenda.id_paciente WHERE tbl_agenda.servicos='exames' LIMIT 0, 200";
	$result=mysqli_query($con,$query);
	while($row=mysqli_fetch_array($result))
	{
	echo "<tr>
			...
			<td>
	            <form method='post' action='insesao_servicos_exames.php'>
	                <a href='insesao_servicos_exames.php?Id=".$row['Id']."&acao=insencao'' name ='insesao' id='insesaoBtn' data-id='".$row['Id']."' href='javascript:void(0)' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='isenção'>
