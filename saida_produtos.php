...
<div id='page-content' style='margin-top:-18px;'>"; 
		...
		if ($acao == 'tirar_produto') {
			...
			echo "
							<a href='saida_produtos.php' style='padding-right: 10px; float:center; font-size:20px; background-color: #149900; color: #fff;' class='btn small'>
			...
			<form name='#' class='form-bordered' action='".$_SERVER['PHP_SELF']."' method='post'>
			            ...
			            <div class='form-row'>
			                ...
			                <div class='form-input col-md-6'>
			                    <input readonly type='text' value='".$row['descricao']."' name='nome' id='nome'>
			                </div>
			            </div>
			...
			<br>
			<button style='background-color: #149900; color: #fff;' class='btn medium' name='tirar_produtoa'>
	...
	if (isset($_POST['tirar_produtoa'])) {
		...
		$con=connection();
		$query="SELECT * FROM tbl_estoque WHERE id_produto = '$id_produto'";
		$result=mysqli_query($con,$query);
		...
		$query = "INSERT INTO tbl_saida_de_produto VALUES (Null, '$id_produto','$quantidade','$local','$data','$user_id')";
		//var_dump($query);
		...
		else echo "<script>swal('Oops! Ocorreu um erro a efetuar a saída de um produto','','error');</script>";
		}
	}	
...
<table class='table' id='example1'>
	...
	echo "<tr>
			...
			<td>
			<form method='get' action='saida_produtos.php'>
				<a href='saida_produtos.php?Id=".$row['id_produto']."&acao=tirar_produto ' class='btn small bg-red tooltip-button' data-placement='top' title='Tirar Produto'>
