...
echo "
	<div id='page-content-wrapper'>
		...
		<h3>Faturação de estomatologia</h3>
	</div>
	...
	<div id='page-content' style='margin-top:-18px;'>"; 
		if(isset($_POST['isencao']))
		{
			...
			$query="INSERT INTO tbl_facturacao VALUES (Null, '$n_factura','$id_paciente','$nome_paciente','$funcionario','$idade','$telefone','estomatologia','$servico','$total','$troco','$totalRecebido','$tipo_pagamento','$referencia', 0, 0, 0, 0, '$data','$data_simples')";
			mysqli_query($con,"SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
			$a = mysqli_query($con,$query) ? true : false ;
			if($a){
				mysqli_query($con,"UPDATE tbl_paciente SET pagou_estomatologia=1 WHERE Id = '$id_paciente'");
			    echo "	<div class='row'>
			                <div class='col-md-6'  style='margin-top:20%;margin-left:25%;'>
			                    <div class='infobox success-bg mrg0A text-center'>
			                        ...
			                        <a href='imprimir/imprimir_fatura_estomatologia.php?id=".$id_paciente."&uid=".$funcionario."' target='_blank' class='btn primary-bg large' name='isencao'>
									    <span style='color:white;'class='button-content'>Gerar Factura</span>
									</a>
			                        <a href='faturar_estomatologia.php' class='btn primary-bg large' name='isencao'>
		...
		if(isset($_POST['isencao_p_idade']))
		{
			...
			$data = date('d-m-Y H:i:s');
			$data_simples = datae('d-m-Y');
			
			$con=connection();

			$query="INSERT INTO tbl_facturacao VALUES (Null, '$n_factura','$id_paciente','$nome_paciente','$funcionario','$idade','$telefone','estomatologia','$servico','$total','$troco','$troco','$tipo_pagamento','$referencia', 0, 0, 0, 0, '$data','$data_simples')";
			mysqli_query($con,"SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
			$a = mysqli_query($con,$query) ? true : false ;
			if($a){
				mysqli_query($con,"UPDATE tbl_paciente SET pagou_estomatologia=1 WHERE Id = '$id_paciente'");
		...
		{			
			    ...
			    echo "<div class='row'>
							<div class='col-md-4'  style='text-align: center; box-shadow: 1px 1px 19px 1px gray;margin-top:10%;margin-left:30%; font-size: 20px;'>	
									...
									<form method='post' action='faturar_estomatologia.php'>	
										<input type='hidden' name='id_paciente' value='".$id_paciente."' />
										<input type='hidden' name='n_factura' value='".$n_factura."' />
										<input type='hidden' name='nome_paciente' value='".$nome_paciente."' />
										<input type='hidden' name='idade' value='".$idade."' />
				...
				<form name='#' class='form-bordered' action='".$_SERVER['PHP_SELF']."' method='post'>
							...
							<table class='table' style='width:70%; margin:0 auto;'>
								...
								while($row=mysqli_fetch_array($result))
								{
									...
									foreach ($idS as $n) {
										$queryS='SELECT * FROM `tbl_servicos_estomatologia` WHERE nome = "'.$n.'"';
										//var_dump($queryS);
...
echo "
	<table class='table' id='example1'>
		...
		$query = "SELECT 
					...
					WHERE (
						servicos = 'estomatologia' OR 
						servicos = 'estomatologia,estomatologia' OR 
						servicos = 'estomatologia,gerais' OR 
						servicos = 'estomatologia,exames' OR 
						servicos = 'estomatologia,exames,exames' OR 
						servicos = 'estomatologia,consultas' OR 
						servicos = 'estomatologia,consultas,consultas' OR 
						servicos = 'estomatologia,estomatologia,consultas,consultas' OR 
						servicos = 'estomatologia,estomatologia,estomatologia') 
						servicos = 'estomatologia,estomatologia,estomatologia,estomatologia') 
					AND 
					t.agendado=1 
					AND 
					t.pagou_estomatologia=0";
		$result=mysqli_query($con,$query);
		while($row=mysqli_fetch_array($result))
		{
		echo "<tr>
				...
				<td>".$row['idade']."</td>
				<td>
				<form method='get' action='faturar_estomatologia.php'>
				";
				if($loggedInUser->title == 'admin') {
					echo "
					<a href='faturar_estomatologia.php?Id=".$row['Id']."&acao=editar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Editar'>
						<i class='glyph-icon icon-edit'></i>
					</a>";
				}
					echo "
					<a href='faturar_estomatologia.php?Id=".$row['Id']."&acao=facturar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Facturar'>
						<i class='glyph-icon icon-money'></i>
					</a>
				</form>
				</td>
