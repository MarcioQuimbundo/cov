 	...
 	if(isset($_GET['Id']))
        {
	        ...
	        if ($acao == 'devolver') {
				...
				$queryInsert = "INSERT INTO tbl_devolucao_servicos_devolucao_servicos_exames VALUES ('', '$n_fatura', '$paciente', '$servico', '$total', '$data_devolucao')";
				...
				if ($resultadoInsert) {
					...
					$queryUpdate = "UPDATE `tbl_facturacao` SET `d_servicos_exames` = '1' WHERE `tbl_facturacao`.`facturacao_id` = '$n_fatura'";
...
<table class='table' id='example1'>
	...
	$con=connection();
		...
		$queryIdDevol="SELECT n_fatura FROM tbl_devolucao_servicos_devolucao_servicos_exames";
		...
		$query = "SELECT 
					...
					WHERE FIND_IN_SET('$nomeConsulta',tbl_facturacao.servico) AND tbl_facturacao.d_servicos_exames = 0";
	...
	echo "<tr>
			...
			<td>".$row['data']."</td>
			<td>
            <form method='get' action='devolucao_servicos_exames.php'>
...
<script>
	...
	function Devolver(facturaId, motivo){
						...
						xmlhttp.open("GET","AjaxFazerDevolucaoExame.php?facturaId="+facturaId+"&motivo="+motivo, false);
						xmlhttp.send(null);
							//alert(xmlhttp.responseText);
						if (xmlhttp.responseText == '1'){
							swal('Devolução efectuada com sucesso!', '', 'success');
							setTimeout("location.href = 'devolucao_servicos_exames.php';", 2500);
						} 							
						else
