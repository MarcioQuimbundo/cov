...
<div id='page-content' style='margin-top:-18px;'>"; 
        ...
        if ($acao == 'apagar') {
            $query="DELETE FROM tbl_servicos_consultas WHERE id = '$Id'";
            $a=mysqli_query($con,$query) ? true : false ;
            if($a){
                    echo "<script>swal('Serviço removido com sucesso', '', 'success');</script>";
                }
            else echo "<script>swal('Oops, ocorreu um erro ao remover o exame','','error');</script>";
        }
        if ($acao == 'editar') {
            $query="SELECT * FROM tbl_servicos_consultas WHERE id   = '$Id'";
            $result=mysqli_query($con,$query);
            while($row=mysqli_fetch_array($result))
            echo "
            <h2>Informações Iniciais</h2>
							<a href='servicos_consultas.php' style='padding-right: 10px; float:center; font-size:20px; background-color: #149900; color: #fff;' class='btn small'>
...
die();
        ...
        if (isset($_POST['voltar'])) {
            echo "<script>document.location.href = 'servicos_consultas.php';</script>";
        }
        ...
        if (isset($_POST['editar'])) {
            ...
            $query = "UPDATE tbl_servicos_consultas SET nome='$servico', preco='$preco', descricao='$descricao' WHERE id   = '$id'";
            var_dump($query);
            $a=mysqli_query($con,$query) ? true : false ;
            if($a){
            ...
            else echo "<script>swal('Oops! Ocorreu um erro ao atualizar o serviço','','error');</script>";
            }
        }   
		if (isset($_POST['add'])) {
			$servico=$_POST['servico'];	
			...
			if (empty($servico) || empty($preco) || empty($descricao) ) {
                echo "<script>swal('Preencha todos os campos','','warning');</script>";
		    }else{
		    $con=connection();

    		$query="INSERT INTO tbl_servicos_consultas VALUES (Null, '$servico','$preco','$descricao','1','$data_criada')";
            //var_dump($query);
		    ...
		    else echo "<script>swal('Oops! erro ao adicionar um serviço','','error');</script>";
		    }
		}	
...
<table class='table' id='example1'>
	...
	<tbody>";
	$con=connection();
	$query="SELECT * FROM tbl_servicos_consultas";
	$result=mysqli_query($con,$query);
	while($row=mysqli_fetch_array($result))
	{
	echo "<tr>
			<td>
                <a href='servicos_consultas.php?Id=".$row['id']."&acao=ver '>".$row['id']."</a>
            </td>
            <td>
                <a href='servicos_consultas.php?Id=".$row['id']."&acao=ver '>".$row['nome']."</a>
            </td>
            <td>
                <a href='servicos_consultas.php?Id=".$row['id']."&acao=ver '>".dinheiro($row['preco'])."</a>
            </td>
            <td>
            <form method='get' action='servicos_consultas.php'>
                <a href='servicos_consultas.php?Id=".$row['id']."&acao=editar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Editar'>
                    <i class='glyph-icon icon-edit'></i>
                </a>
                <a href='servicos_consultas.php?Id=".$row['id']."&acao=ver ' class='btn small bg-gray tooltip-button' data-placement='top' title='Ver'>
                    <i class='glyph-icon icon-eye'></i>
                </a>
                <a href='servicos_consultas.php?Id=".$row['id']."&acao=apagar ' class='btn small bg-red tooltip-button' data-placement='top' title='Apagar'>
	...
	<form method='post' action='servicos_consultas.php'>
	    <a title='Voltar' href='servicos_consultas.php'>
