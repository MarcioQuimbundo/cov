	...
	if ($acao == 'editar') {
			...
			<form name='#' class='form-bordered' action='".$_SERVER['PHP_SELF']."' method='post'>
			            ...
			            <div class='form-row'>
			                ...
			                <div class='form-input col-md-6'>
			                    <input required='required' type='text' value='".$row['email']."' name='endereco' id='endereco' onfocus='searchVendor()'>
			                </div>
			            </div>
			            ...
			            <div class='form-row'>
			                ...
			                <div class='form-input col-md-6'>
			                    <input type='tipo_id' value='".$row['tipo_id']."' name='tipo_id' id='tipo_id' onfocus='searchVendor()'>
			                </div>
			            </div>
			...
			<h2>Informações Pessoais</h2><br>
					...
					<a href='pacientes.php?Id=".$_GET['Id']."&acao=agendar ' style='float:center, font-size:20px; background-color: #149900; color: #fff;' class='btn small'  title='Editar'>
		    ...
		    if (empty($nome) || empty($data_nasc) || empty($genero) || empty($endereco) ) {
		    	...
		    	$con=connection();
					    ...
					    VALUES (
							...
							mysqli_query($con,"SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
				    $a = mysqli_query($con,$query) ? true : false ;
				    if($a) echo "<script>swal('Paciente Cadastrado Com Sucesso','','success');</script>";
		    		else echo "<script>swal('Ops! Ocorreu um erro ao cadastrar o paciente','','error');</script>";
		    }
	}	
	...
	if (isset($_POST['editarr'])) {
		    ...
		    if (empty($nome) || empty($data_nasc) || empty($genero)) {
		    	...
		    	$con=connection();					    
				    ...
				    $a=mysqli_query($con,$query) ? true : false ;
				    if($a) echo "<script>swal('Paciente atualizado Com Sucesso','','success');</script>";
		    		else echo "<script>swal('Ops! Ocorreu um erro ao atualizar o paciente','','error');</script>";
				    }
				}
...
<table class='table' id='example1'>
	...
	<tbody>";
	$con=connection();
	$query="SELECT Id, nome, (TIMESTAMPDIFF(YEAR, data_nasc, CURDATE())) AS idade, genero, telefone FROM tbl_paciente LIMIT 0, 200";
	$result=mysqli_query($con,$query);
	while($row=mysqli_fetch_array($result))
	{
	echo "<tr>
			<td>".$row['Id']."</td>
			<td>
				<a href='pacientes_cadastrados.php?Id=".$row['Id']."&acao=ver '>".utf8_encode($row['nome'])."</a>
			</td>
			<td>".$row['idade']."</td>
			<td>".$row['genero']."</td>
			<td>".$row['telefone']."</td>
			<td>
			<form method='get' action='pacientes_cadastrados.php'>
			";
				echo "
				<a href='pacientes_cadastrados.php?Id=".$row['Id']."&acao=editar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Editar'>
			        <i class='glyph-icon icon-edit'></i>
				</a>";
				echo "
			    <a href='pacientes_cadastrados.php?Id=".$row['Id']."&acao=ver ' class='btn small bg-gray tooltip-button' data-placement='top' title='Ver'>
			        <i class='glyph-icon icon-eye'></i>
			    </a>
			    <a href='pacientes_cadastrados.php?Id=".$row['Id']."&acao=agendar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Agendar'>
			        <i class='glyph-icon icon-calendar'></i>
			    </a>
			    <a href='pacientes_cadastrados.php?Id=".$row['Id']."&acao=faturar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Faturar'>
			        <i class='glyph-icon icon-money'></i>
			    </a>
			</form>
			</td>
	...
	<form method='post' action='pacientes_cadastrados.php'>
	    <a title='Novo Paciente' href='pacientes_cadastrados.php'>
