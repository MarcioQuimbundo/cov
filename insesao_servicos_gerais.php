...
echo "
	<div id='page-content-wrapper'>
		...
		<h3>Isenção dos Serviços Gerais</h3>
	</div>
	...
	if (isset($_POST['dar_entradaa'])) {
		...
		$query = "INSERT INTO tbl_isencao_servicos_gerais VALUES(NULL,'$idPaciente','$tipo_servicos','$preco','$quantidade','$total','$descricao','$user_id', '$data_isentada')";	
		var_dump($query);
		$result = mysqli_query($con,$query);
		$q = "SELECT * FROM tbl_isencao_servicos_gerais";	
		...
		if ($result) {
			echo "<script>swal('Paciente Insentado com sucesso','','success');</script>";
			 echo "	<div class='row'>
			                <div class='col-md-6'  style='margin-top:15%;margin-left:25%;'>
			                    <div class='infobox success-bg mrg0A text-center'>
			                        ...
			                        <a href='imprimir/imprimir_isencao_servicos_gerais.php?Id=".$idPaciente."&uid=".$count."' target='_blank' class='btn primary-bg large' name='isencao'>
 	...
 	if(isset($_GET['Id']))
        {
	        ...
	        if ($acao == 'insencao') {
				...
				echo "
			<a href='insesao_servicos_gerais.php' style='padding-right: 10px; float:center; font-size:20px; background-color: #149900; color: #fff;' class='btn small'>
			              ...
			              ";?>
		              			...
		              			<div class='form-input col-md-6'>
									<select required='required' id='tipo_servicos' onchange="mudaPreco();" name='tipo_servicos' class='form-control' style="font-size: 14px;">
										...
										<?php  
											$query="SELECT id, nome FROM tbl_servicos_gerais";
											$result=mysqli_query($con,$query);
											while($row=mysqli_fetch_array($result))
											{
										?>
										<option value=<?="gerais:".$row['id']; ?> ><?=$row['nome']; ?></option>        
										<?php } ?>                           
									</select>
								</div>
							</div>
...
<table class='table' id='example1'>
	...
	$con=connection();
	$query="SELECT Id, nome, (TIMESTAMPDIFF(YEAR, data_nasc, CURDATE())) AS idade, genero, telefone FROM tbl_paciente inner join tbl_agenda ON tbl_paciente.Id = tbl_agenda.id_paciente WHERE tbl_agenda.servicos='gerais' LIMIT 0, 200";
	$result=mysqli_query($con,$query);
	while($row=mysqli_fetch_array($result))
	{
	echo "<tr>
			...
			<td>
	            <form method='post' action='insesao_servicos_gerais.php'>
	                <a href='insesao_servicos_gerais.php?Id=".$row['Id']."&acao=insencao' name ='insesao' id='insesaoBtn' data-id='".$row['Id']."' href='javascript:void(0)' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='insentar'>
