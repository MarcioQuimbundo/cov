...
<h3>Serviços de Estomatologia</h3>
</div>
...
<div id='page-content' style='margin-top:-18px;'>"; 
        ...
        if ($acao == 'apagar') {
            $query="DELETE FROM tbl_servicos_estomatologia WHERE id='$Id'";
            $a=mysqli_query($con,$query) ? true : false ;
            if($a){
                    echo "<script>swal('Serviço removido com sucesso', '', 'success');</script>";
                }
            else echo "<script>swal('Oops! Ocorreu Um Erro Ao Remover o Serviço', '', 'error');</script>";
        }
        if ($acao == 'editar') {
            $query="SELECT * FROM tbl_servicos_estomatologia WHERE id = '$Id'";
            $result=mysqli_query($con,$query);
            while($row=mysqli_fetch_array($result))
            echo "
            <h2>Informações Iniciais</h2>
							<a href='servicos_estomatologia.php' style='padding-right: 10px; float:center; font-size:20px; background-color: #149900; color: #fff;' class='btn small'>
...
die();
        ...
        if (isset($_POST['voltar'])) {
            echo "<script>document.location.href = 'servicos_estomatologia.php';</script>";
        }
        ...
        if (isset($_POST['editar'])) {
            ...
            $query = "UPDATE tbl_servicos_estomatologia SET nome='$servico', preco='$preco', descricao='$descricao' WHERE id = '$id'";
            var_dump($query);
            $a=mysqli_query($con,$query) ? true : false ;
            if($a){
		...
		if (isset($_POST['add'])) {
    		...
    		$query="INSERT INTO tbl_servicos_estomatologia VALUES (null , '$servico','$preco','$descricao','1','$data_criada')";
		    ...
		    else echo "<script>swal('Oops! Ocorreu Um Erro Ao Adicionar o Serviço', '', 'error');</script>";
		    }
		}	
...
<table class='table' id='example1'>
	...
	<tbody>";
	$con=connection();
	$query="SELECT * FROM tbl_servicos_estomatologia";
	$result=mysqli_query($con,$query);
	while($row=mysqli_fetch_array($result))
	{
	echo "<tr>
			<td>
                <a href='servicos_estomatologia.php?Id=".$row['id']."&acao=ver '>".$row['id']."</a>
            </td>
            <td>
                <a href='servicos_estomatologia.php?Id=".$row['id']."&acao=ver '>".$row['nome']."</a>
            </td>
            <td>
                <a href='servicos_estomatologia.php?Id=".$row['id']."&acao=ver '>".dinheiro($row['preco'])."</a>
            </td>
            <td>
            <form method='get' action='servicos_estomatologia.php'>
                <a href='servicos_estomatologia.php?Id=".$row['id']."&acao=editar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Editar'>
                    <i class='glyph-icon icon-edit'></i>
                </a>
                <a href='servicos_estomatologia.php?Id=".$row['id']."&acao=ver ' class='btn small bg-gray tooltip-button' data-placement='top' title='Ver'>
                    <i class='glyph-icon icon-eye'></i>
                </a>
                <a href='servicos_estomatologia.php?Id=".$row['id']."&acao=apagar ' class='btn small bg-red tooltip-button' data-placement='top' title='Apagar'>
	...
	<form method='post' action='servicos_estomatologia.php'>
	    <a title='Voltar' href='servicos_estomatologia.php'>
