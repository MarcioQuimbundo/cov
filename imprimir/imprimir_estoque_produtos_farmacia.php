...
class myPDF extends FPDF {
	function header () {
		...
		$this->Cell(276, 35, utf8_decode('LISTA DOS PRODUTOS FARMÁCIA'), 0, 0, 'C');
		$this->Ln();
	...
	function viewTable ($db) {
		$this->SetFont('Times', 'B', 12);
		$stmt = $db->query('SELECT * FROM tbl_estoque_farmacia');
		while ($data = $stmt->fetch(PDO::FETCH_OBJ)) {
			$id_produto = $data->id_produto;
				$stmtM = $db->query("SELECT * FROM tbl_produto_farmacia WHERE id_produto ='$id_produto'");
				while ($rowM = $stmtM->fetch(PDO::FETCH_OBJ)) {
					$this->Cell(90, 10, utf8_decode($rowM->nome_comercial), 1, 0, 'L');
				}
