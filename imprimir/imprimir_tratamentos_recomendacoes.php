...
class myPDF extends FPDF {
	...
	function viewTable ($db) {
		...
		$id = $_GET['id'];
		$stmt = $db->query("SELECT id_paciente, id_medico, tratamentos, data_criada, recomendacoes FROM tbl_tratamentos_recomendacoes WHERE id_tratamento_rec = $id");
		//var_dump($stmt);
		$dados = $stmt->fetch(PDO::FETCH_OBJ);
		$dadosrec = $dados->recomendacoes;
		...
		$this->Cell(1, 25, utf8_decode(' Data: '.$dados->data_criada), 0, 0, 'L');
		...
		$this->Cell(-190, 95, utf8_decode('TRATAMENTOS '), 0, 0, 'C');
		$this->SetFont('Times', '', 12);
		$this->SetXY(10,100);
		$dados = utf8_decode($dados->tratamentos);
		
		$this->MultiCell(0,5,$dados,'J');
		$this->SetFont('Times', 'B', 12);
		$this->Cell(188, 135, utf8_decode('RECOMENDAÇÕES '), 0, 0, 'C');
		$this->SetFont('Times', '', 12);
		$this->SetXY(10,185);
		$this->MultiCell(0,5,$dadosrec,'J');
