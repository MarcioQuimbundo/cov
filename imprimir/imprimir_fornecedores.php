...
class myPDF extends FPDF {
	function header () {
		...
		$this->Cell(276, 35, utf8_decode('LISTA DOS FORNECEDORES'), 0, 0, 'C');
		$this->Ln();
	...
	function headerTable () {
		...
		$this->Cell(60, 10, utf8_decode('Descrição'), 1, 0, 'C');
		$this->Ln();
	}

	function viewTable ($db) {
		$this->SetFont('Times', 'B', 12);
		$stmt = $db->query('SELECT * FROM tbl_fornecedores');
		while ($data = $stmt->fetch(PDO::FETCH_OBJ)) {
			...
			$this->Cell(60, 10, utf8_decode($data->descricao), 1, 0, 'L');
			$this->Ln();
		}
	}
}
