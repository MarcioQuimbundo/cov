...
class myPDF extends FPDF {
	...
	function headerTable () {
		...
		$this->Cell(150, 10, utf8_decode('Funcionário'), 1, 0, 'C');
		$this->Cell(130, 10, utf8_decode('Total Aberto'), 1, 0, 'C');
		$this->Ln();
	}
	...
	function viewTable ($db) {
		...
		while ($data = $stmt->fetch(PDO::FETCH_OBJ)) {
			$this->Cell(150, 10, utf8_decode($data->nome_funcionario), 1, 0, 'L');
			$this->Cell(130, 10, number_format($data->TotalAbertura, 2). ' kz', 1, 0, 'L');
			$this->Ln();
		}
	}
}
