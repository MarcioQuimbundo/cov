...
class myPDF extends FPDF {
	...
	function footer () {
		...
		$db = new PDO('mysql:host=localhost;dbname=cov;charset=utf8','root','ANGO_covdb157016');
		$stmtI = $db->query("SELECT tbl_users.display_name FROM tbl_isencao_servicos_exames inner join tbl_users on user_id=tbl_users.id WHERE id_paciente=$id_paciente");
	...
	function viewTable ($db) {
		...
		$stmtI = $db->query("SELECT * FROM tbl_isencao_servicos_exames INNER JOIN tbl_servicos_exames  ON tbl_isencao_servicos_exames.servicos=tbl_servicos_exames.id WHERE tbl_isencao_servicos_exames.id = $id_comprovativo");
		$dadosI =$stmtI->fetch(PDO::FETCH_OBJ);
