...
class myPDF extends FPDF {
	function header () {
		...
		$this->Cell(276, 35, utf8_decode('LISTA DOS UTILIZADORES DO SISTEMA'), 0, 0, 'C');
		$this->Ln();
	...
	function headerTable () {
		...
		$this->Cell(100, 10, utf8_decode('Nome à mostrar'), 1, 0, 'C');
		$this->Cell(90, 10, 'Categoria', 1, 0, 'C');
		$this->Ln();
	}
	...
	function viewTable ($db) {
		...
		while ($data = $stmt->fetch(PDO::FETCH_OBJ)) {
			$this->Cell(90, 10, $data->user_name, 1, 0, 'L');
			$this->Cell(100, 10, utf8_decode($data->display_name), 1, 0, 'L');
			$this->Cell(90, 10, $data->title, 1, 0, 'L');
			$this->Ln();
		}
	}
}
