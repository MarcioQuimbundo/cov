...
class myPDF extends FPDF {
	function header () {
		...
		if (count($servicos) == 1) {
				if (strlen($servicos[0]) > 20) {
					$this->SetFont('Arial', '', 7);
					$this->Cell( 272, 135, utf8_decode($servicos[0]), 0, 0, 'C');
				} else {
				...
				$stmtS = $db->query("SELECT preco FROM tbl_servicos_gerais WHERE nome = '$servico'");
				...
				$servico1 = $servicos[0];
				$stmtS = $db->query("SELECT preco FROM tbl_servicos_gerais WHERE TRIM(nome) = '$servico1'");
				$dadosS = $stmtS->fetch(PDO::FETCH_OBJ);
				//var_dump($servico1);
				$this->Cell( 257, 135, $dadosS->preco, 0, 0, 'C');
				$this->Cell( -220, 135, '0%', 0, 0, 'C');
				...
				$servico2 = $servicos[1];
				$stmtS2 = $db->query("SELECT preco FROM tbl_servicos_gerais WHERE nome = '$servico2'");
				...
				$servico1 = $servicos[0];
				$stmtS = $db->query("SELECT preco FROM tbl_servicos_gerais WHERE nome = '$servico1'");
				$dadosS = $stmtS->fetch(PDO::FETCH_OBJ);
				...
				$servico3 = $servicos[2];
				$stmtS3 = $db->query("SELECT * FROM tbl_servicos_gerais WHERE nome = '$servico3'");
