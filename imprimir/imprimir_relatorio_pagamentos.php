...
class myPDF extends FPDF {
	function header () {
		...
		$this->Cell(276, 35, utf8_decode('HISTÓRICO DE VENDAS'), 0, 0, 'C');
		$this->Ln();
	...
	function headerTable () {
		...
		$this->Cell(35, 10, utf8_decode('Pagamento'), 1, 0, 'C');
		$this->Cell(30, 10, utf8_decode('Total'), 1, 0, 'C');
		$this->Cell(25, 10, utf8_decode('Data'), 1, 0, 'C');
		$this->Ln();
	}

	function viewTable ($db) {
		$this->SetFont('Times', 'B', 12);
		$stmt = $db->query('SELECT * FROM tbl_facturacao INNER JOIN tbl_users ON(tbl_facturacao.funcionario = tbl_users.id)');
		while ($data = $stmt->fetch(PDO::FETCH_OBJ)) {
			...
			$this->Cell(35, 10, utf8_decode($data->tipo_pagamento), 1, 0, 'L');
			$this->Cell(30, 10, ($data->total).' kz', 1, 0, 'L');
			$this->Cell(25, 10, utf8_decode($data->data), 1, 0, 'L');
			$this->Ln();
		}
	}
}
