...
echo "
	...
	<div id='page-content' style='margin-top:-18px;'>"; 
		if(isset($_POST['isencao']))
		{
			...
			$query="INSERT INTO tbl_facturacao VALUES (Null, '$n_factura','$id_paciente','$nome_paciente','$funcionario','$idade','$telefone','consultas','$servico','$total','$troco','$totalRecebido','$tipo_pagamento','$referencia', 0, 0, 0, 0, '$data','$data_simples')";
			    ...
			    echo "	<div class='row'>
			                <div class='col-md-6'  style='margin-top:20%;margin-left:25%;'>
			                    <div class='infobox success-bg mrg0A text-center'>
			                        ...
			                        <a href='imprimir/imprimir_fatura_consultas.php?id=".$id_paciente."&uid=".$funcionario."' target='_blank' class='btn primary-bg large' name='isencao'>
									    <span style='color:white;'class='button-content'>Gerar Factura</span>
									</a>
			                        <a href='faturar_consultas.php' class='btn primary-bg large' name='isencao'>
		...
		if(isset($_POST['isencao_p_idade']))
		{
			...
			$query="INSERT INTO tbl_facturacao VALUES (Null, '$n_factura','$id_paciente','$nome_paciente','$funcionario','$idade','$telefone','consultas','$servico','$total','$troco','$troco','$tipo_pagamento','$referencia', 0, 0, 0, 0, '$data','$data_simples')";
			//var_dump($query);
			mysqli_query($con,"SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
			$a = mysqli_query($con,$query) ? true : false ;
			if($a){
		...
		if(isset($_POST['teste']))
		{
			...
			$tipo_pagamento ='Multicaixa';
			$data = date('d-m-Y H:i:s');
			$data_simples = date('d-m-Y');

			$con=connection();								

			$query="INSERT INTO tbl_facturacao VALUES (Null, 
					'$n_factura',
					'$id_paciente',
					'$nome_paciente',
					'$funcionario',
		...
		{			
			    ...
			    echo "<div class='row'>
							<div class='col-md-4'  style='text-align: center; box-shadow: 1px 1px 19px 1px gray;margin-top:10%;margin-left:30%; font-size: 20px;'>	
									...
									<form method='post' action='faturar_consultas.php'>	
		...
		if(isset($_GET['Id']))
		{
		    ...
		    if ($acao == 'facturar') {
				...
				<form name='#' class='form-bordered' action='".$_SERVER['PHP_SELF']."' method='post'>
							...
							<br>
							<h2 class='text-center'>Consulta por facturar</h2><br>
							<table class='table' style='width:70%; margin:0 auto;'>
								...
								while($row=mysqli_fetch_array($result))
								{
									...
									foreach ($idS as $n) {
										$queryS='SELECT * FROM `tbl_servicos_consultas` WHERE nome = "'.$n.'"';
										//var_dump($queryS);
...
echo "
	<table class='table' id='example1'>
		...
		$queryNome="SELECT nome FROM tbl_servicos_consultas";
		...
		$query = "SELECT 
					...
					WHERE 
					(
					servicos = 'consultas' OR 
					servicos = 'consultas,consultas' OR 
					servicos = 'consultas,consultas,consultas' OR 
					servicos = 'consultas,consultas,consultas,consultas'OR 
					servicos = 'consultas,estomatologia'OR 
					servicos = 'consultas,estomatologia,estomatologia'OR 
					servicos = 'consultas,consultas,estomatologia,estomatologia'OR 
					servicos = 'consultas,consultas,estomatologia,estomatologia,estomatologia'OR 
					servicos = 'consultas,consultas,consultas,estomatologia,estomatologia'OR 
					servicos = 'consultas,consultas,estomatologia'OR 
					servicos = 'consultas,consultas,consultas,consultas,consultas')  AND  
					(((TIMESTAMPDIFF(YEAR, t.data_nasc, CURDATE())) >= 9) AND 
					((TIMESTAMPDIFF(YEAR, t.data_nasc, CURDATE())) <= 60)) AND 
					t.agendado=1 AND 
					t.pagou=0";
		...
		echo "<tr>
				...
				<td>".$row['idade']."</td>
				<td>
				<form method='get' action='faturar_consultas.php'>
				";
				if($loggedInUser->title == 'admin') {
					echo "
					<a href='faturar_consultas.php?Id=".$row['Id']."&acao=editar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Editar'>
						<i class='glyph-icon icon-edit'></i>
					</a>";
				}
					echo "
					<a href='faturar_consultas.php?Id=".$row['Id']."&acao=facturar ' class='btn small bg-blue-alt tooltip-button' data-placement='top' title='Facturar'>
						<i class='glyph-icon icon-money'></i>
					</a>
				</form>
				</td>
